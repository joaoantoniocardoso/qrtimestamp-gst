...
fn main() {
    ...
    //      2. We are skipping the last 20 frames because they are the ones that were retained in the queue,
    //      thus as there is no more frames coming in, they are released faster, which lowers their latency
    let latencies = latencies[1..=latencies.len() - queue_buffers]
        .iter()
        .map(|i| *i as f64)
        .collect::<Vec<f64>>();
    dbg!(&latencies);
    ...
    let latency_min = latencies.clone().min();
    let latency_max = latencies.clone().max();
    let latency_mean = latencies.clone().mean();
    let latency_std_dev = latencies.clone().std_dev();
    let latency_variance = latencies.clone().variance();
    let jitter_min = jitters.clone().min();
    let jitter_max = jitters.clone().max();
    let jitter_mean = jitters.clone().mean();
    let jitter_std_dev = jitters.clone().std_dev();
    let jitter_variance = jitters.clone().variance();
